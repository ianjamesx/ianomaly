...
void initMap(double *pairs, int allpairsize, int nk, int ops, int threadsperpair){
  ...
  if(thread % 2 != 0) return; //<-- highly innefficient
...
double getMemoryAllocation(int recordsize){
  ...
  int sizeperrecord = (nk * doublesize) + (recordsize * doublesize); //also memory must be stored for phase1 op
  ...
  cout << "Recommended record max: " << (maxrecords * .8) << endl;
